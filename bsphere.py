...
class arrow():
    def __init__(self, tail, head, color): 
        self.tail = tail
        self.head = head
        self.color = color 
    def pts(self):
        # -X and Y data are switched for plotting purposes
        return [[self.tail.y, self.head.y], [-self.tail.x, -self.head.x], [self.tail.z, self.head.z] ]
...
class Bloch():
    ...
    def __init__(self, fig=None, axes=None):
        ...
        self.point_color = ['k']
        # Size of point markers, default = [25, 32, 35, 45]
        self.point_size = [25]
    ...
    def add_arrow(self, tail, head, color):
        """Add a an arrow to be plotted

        Parameters
        ----------
        tail : vec3 of arrow tail
        head : vec3 of arrow head

        """
        arr = arrow( tail, head, color) 
    ...
    def plot_arrows(self):
        ...
        if len(self.arrows) > 0:
            for arr in self.arrows: 
                a = Arrow3D( *arr.pts() , 
                             mutation_scale=self.vector_mutation, lw=self.vector_width, 
                             arrowstyle=self.vector_style,
                             color=arr.color)
                self.axes.add_artist(a)
