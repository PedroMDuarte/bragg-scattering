...
def mainloop(state, plane, fileprefix="", debug=1):
	...
	# constants
	#fileprefix="test-"
	comment="Halfway detuned images - Version 7\n" #comment for csv file
	...
	kout = kouthat*kmag
	deltak = kout-kin
	DW = exp(-1.*dot(deltak,deltak)*sholength**2./2.) #Debye-Waller factor, assuming harmonic oscillator approximation
	#print "DW: %g" % DW
	...
	#lineshape = gamma/2.*(detuning + 0.5j*gamma)/(detuning*detuning+rabi**2/2+gamma**2/4)
	# Next line from Als-Nielsen
	lineshape = (3.*wavelength/4./pi)*0.5*gamma*(detuning - 0.5j*gamma)/(detuning*detuning+gamma*gamma/4.)
	#atten=lineshape*sigma/latticespacing/latticespacing
	polterm=asarray([0,sqrt((3+cos(2*ccdtheta))/8.),sqrt((3+cos(2*ccdtheta))/8.)])
	# polarization term: above expression only holds for incoming light linearly polarized in xy plane
	atten=lineshape * DW * polterm # multiply by some constants and the polarization factor
	#print "atten: %g" % atten
	...
	#print "localphase: %g" % localphase
	# !!rewrite phasesum to be a sum over electric field.  include polarization
	# Prefactor to normalize ccd pixel values
	ccdnorm = flux*ccdpixelarea*exposuretime #normalize to photons/pixel.  account for geometry, etc.
	#ccdnorm = 1 #gives units as fraction of incoming intensity
	#print "ccdnorm: %g" % ccdnorm
		...
		for x in range(0,shape[0]):
			for y in range(0,shape[1]):
				for z in range(0,shape[2]):
					phase += localphase[x,y,z]*exp(1.j*kmag*sqrt((a[0]-b[0,x,y,z])**2. + (a[1]-b[1,x,y,z])**2. + (a[2]-b[2,x,y,z])**2.))/sqrt((a[0]-b[0,x,y,z])**2. + (a[1]-b[1,x,y,z])**2. + (a[2]-b[2,x,y,z])**2.)
		return phase
...
def main(argv=None):
	...
	for state in range(0,3):
		for plane in range(0,2):
			mainloop(state, plane, fileprefix="20090415-", debug=0)
	#mainloop(0,0,"fix-",0)
